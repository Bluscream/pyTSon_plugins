...
class faker(ts3plugin):
    ...
    commandKeyword = ""
    infoTitle = None
    iconPath = path.join(ts3.getPluginPath(), "pyTSon", "scripts", "faker", "icons")
    ...
    def fakeChannel(self, schid, channelID):
        (error, nick) = ts3.getChannelVariableAsString(schid, channelID, ts3defines.ChannelProperties.CHANNEL_NAME)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_NAME,nick)

        (error, phonetic) = ts3.getChannelVariableAsString(schid, channelID, ts3defines.ChannelProperties.CHANNEL_NAME_PHONETIC)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_NAME,phonetic)
        
        (error, pw) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_PASSWORD)
        if pw: ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_PASSWORD,".")

        (error, topic) = ts3.getChannelVariableAsString(schid, channelID, ts3defines.ChannelProperties.CHANNEL_TOPIC)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_TOPIC,topic)

        (error, description) = ts3.getChannelVariableAsString(schid, channelID, ts3defines.ChannelProperties.CHANNEL_DESCRIPTION)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_DESCRIPTION,description)

        (error, neededtp) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_NEEDED_TALK_POWER)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_NEEDED_TALK_POWER,neededtp)

        (error, codec) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_CODEC)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_CODEC,codec)

        (error, quality) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_CODEC_QUALITY)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_CODEC_QUALITY,quality)

        (error, latency) = ts3.getChannelVariableAsString(schid, channelID, ts3defines.ChannelProperties.CHANNEL_CODEC_LATENCY_FACTOR)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_CODEC_LATENCY_FACTOR,latency)

        (error, unencrypted) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_CODEC_IS_UNENCRYPTED)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_CODEC_IS_UNENCRYPTED,unencrypted)

        (error, maxclients) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_MAXCLIENTS)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_MAXCLIENTS,maxclients)

        (error, maxfamilyclients) = ts3.getChannelVariableAsInt(schid, channelID, ts3defines.ChannelProperties.CHANNEL_MAXFAMILYCLIENTS)
        ts3lib.setChannelVariableAsString(sCHID, 0,ts3defines.ChannelProperties.CHANNEL_MAXFAMILYCLIENTS,maxfamilyclients)

        (error, iconid) = ts3.getChannelVariableAsUint64(schid, channelID, ts3defines.ChannelProperties.CHANNEL_ICON_ID)
    ...
    def fakeClient(self, schid, clientID):
        (error, _clid) = ts3.getClientID(schid)
        (error, tnick) = ts3.getClientVariableAsUInt64(schid, clientID, ts3defines.ClientProperties.CLIENT_NICKNAME)
        (error, tnickp) = ts3.getClientVariableAsUInt64(schid, clientID, ts3defines.ClientProperties.CLIENT_NICKNAME_PHONETIC)
