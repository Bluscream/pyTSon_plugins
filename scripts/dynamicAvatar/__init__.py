...
class dynamicAvatar(ts3plugin):
    ...
    def onMenuItemEvent(self, schid, atype, menuItemID, selectedItemID):
        try:
            if atype == ts3defines.PluginMenuType.PLUGIN_MENU_TYPE_GLOBAL:
                if menuItemID == 0:
                    if self.config.get('GENERAL', 'mode') == "url": self.urlAvatar(schid)
                    elif self.config.get('GENERAL', 'mode') == "path": self.pathAvatar(schid)
                elif menuItemID == 1:
                    if self.timer.isActive(): self.timer.stop();ts3lib.printMessageToCurrentTab('Timer stopped!')
                    else: self.timer.start(int(self.config.get('GENERAL', 'refresh'))*1000);ts3lib.printMessageToCurrentTab('Timer started!')
        except:
    ...
    def urlAvatar(self, schid):
        try:
            ...
            self.nwm.get(QNetworkRequest(QUrl(self.config.get('GENERAL', 'imgurl'))))
        except:
            from traceback import format_exc
...
class SettingsDialog(QDialog):
    ...
    def __init__(self,Class,parent=None):
        ...
        setupUi(self, path.join(getPluginPath(), "scripts", "dynamicAvatar", "settings.ui"))
