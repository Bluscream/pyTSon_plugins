...
class quickMod(ts3plugin):
    ...
    hotkeys = [
        ...
        ("selected_to_customBan", "Gives the selected user to the customBan dialog"),
    ...
    sgids = []
    lastchans = {}
    last_joined_server = 0; last_joined_channel = 0; last_talk_power = 0; requested = 0; requestedIP = 0
    retcode = ""
    customBan = None
    ...
    def onHotkeyOrCommandEvent(self, keyword, schid=0):
        try:
            ...
            if keyword == "test":
                msg = ["moveBeforeBan",self.moveBeforeBan,"sgids",self.sgids,"lastchans",self.lastchans,
                      "last_joined_server",self.last_joined_server,"last_joined_channel",self.last_joined_channel,"last_talk_power",self.last_talk_power,
                      "requested",self.requested,"requestedIP",self.requestedIP,"retcode",self.retcode]
                print(msg)
                ...
                if item[1] != ServerTreeItemType.CLIENT:
                    ts3lib.playWaveFile(schid, path.join(self.path,"error.wav"))
                    ts3lib.printMessageToCurrentTab("{}: [color=red]No client with name[/color] {} [color=red]found![/color]".format(self.name, name))
                    return
        ...
        except: print("error");ts3lib.logMessage(format_exc(), ts3defines.LogLevel.LogLevel_ERROR, "pyTSon", 0)
        return True

    def toCustomBan(self, schid, clid):
        active = PluginHost.active
        if not "Custom Ban" in active: ts3lib.printMessageToCurrentTab("%s: Customban plugin not active"%self.name);return
    ...
    def banIP(self, schid, ip):
        ...
        ts3lib.banadd(schid, ip, "", "", 0, self.banReason())
    ...
    def onClientMoveEvent(self, schid, clid, oldChannelID, newChannelID, visibility, moveMessage):
        ...
        if sgids is not None and self.sgids:
            sgids = intList(sgids)
