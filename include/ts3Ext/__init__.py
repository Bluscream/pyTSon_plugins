...
class ts3Server(object):
    ...
    def getUser(self, clid):
        if clid == self.me.clientID: return self.me
        if clid in self._ts3Users: return self._ts3Users[clid]
...
class ts3User(object):
    def __init__(self, server, clid, name=None, uid=None, perm=None):
        if not server: raise ValueError("Missing Server object.")
        if not clid or clid < 0: raise ValueError("Invalid client ID.")
        ...
        server.logMsg("New user object %s:%s" % (server.schid, clid), logLevel.TRACE)
    ...
    @property
    def isAway(self):
        (err, iawa) = ts3lib.getClientVariableAsInt(self.schid, self.clientID, ts3defines.ClientPropertiesRare.CLIENT_AWAY)
    ...
    @property
    def isMuted(self):
        (err, imute) = ts3lib.getClientVariableAsInt(self.schid, self.clientID, ts3defines.ClientProperties.CLIENT_IS_MUTED)
    ...
    def isInputMuted(self):
        (err, imute) = ts3lib.getClientVariableAsInt(self.schid, self.clientID, ts3defines.ClientProperties.CLIENT_INPUT_MUTED)
    ...
    def isOutputMuted(self):
        (err, imute) = ts3lib.getClientVariableAsInt(self.schid, self.clientID, ts3defines.ClientProperties.CLIENT_OUTPUT_MUTED)
    ...
    def isOutputOnlyMuted(self):
        (err, imute) = ts3lib.getClientVariableAsInt(self.schid, self.clientID, ts3defines.ClientProperties.CLIENT_OUTPUTONLY_MUTED)
    ...
    def getServerGroups(self):
        ...
        (err, sgid) = ts3lib.getClientVariableAsString(self.schid, self.clientID, ts3defines.ClientPropertiesRare.CLIENT_SERVERGROUPS)
