...
class notify(ts3plugin):
    ...
    def onConnectStatusChangeEvent(self, schid, newStatus, errorNumber):
        if newStatus == ts3defines.ConnectStatus.STATUS_CONNECTION_ESTABLISHED:
            ...
            if error == ts3defines.ERROR_ok and self.parseVersion(targetVer) < self.versions["serverrev"]:
            ...
            if _uid in self.blacklist: self.toggle = False
    ...
    def onUpdateClientEvent(self, schid, clientID, invokerID, invokerName, invokerUniqueIdentifier):
        ...
        if self.checkOutdatedVersion:
            ...
            if error == ts3defines.ERROR_ok and self.parseVersion(targetVer) < self.versions["clientrev"]:
                if country in ["DE", "CH", "AT"]:
                    msg = """[color=red]Hinweis[/color]:
Du nutzt eine veraltete Version von Teamspeak.
Bitte aktualisiere deinen Client auf Version [color=blue]%s[/color] um mögliche Sicherheitsrisiken auszuschliessen.
Um zu updaten gehe einfach auf "Hilfe" => "Nach Aktualisierung suchen"
                    """
                else:
                    msg = """[color=red]Attention[/color]:
                   
You are using a outdated version of Teamspeak.
Your version is very likely vulnerable to several exploits.
Please update your Client to Version [color=blue]%s[/color]
To update click on "Help" => "Check for Update
                    """
                ts3.requestSendPrivateTextMsg(schid, msg % self.versions["clientver"], clientID)
                _done = True
...
You can read them by clicking on \"Tools\" => \"Offline Messages\" or pressing [CTRL]+[O] on your keyboard.
    ...
    def parseVersion(ver):
        return int(ver.split("Build: ")[1].split("]")[0])
